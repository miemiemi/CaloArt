...
This file is vendored from the vit4hep `experiments.calo_utils.ugr_evaluation`
implementation, with only local import-path adjustments.

Within this repository we keep this variant only to support the FPD/KPD feature
pipeline in `src.evaluation.fpd_kpd`. It is not intended to be a behaviorally
identical replacement for the official `cc_metrics.HighLevelFeatures` module.
"""
...
class HighLevelFeatures:
    """Computes high-level features for the vendored vit4hep FPD/KPD pipeline."""
    ...
    def __init__(self, particle, filename="binning.xml"):
        ...
        self.num_alpha = [
            len(xml.alphaListPerLayer[idx][0])
            for idx, redge in enumerate(xml.r_edges)
            if len(redge) > 1
        ]
        self.E_tot = None
        self.E_layers = {}
    ...
    def _DrawShower(self, data, filename, title):
        ...
        if self.particle == "electron":
            ...
            wdth = str(len(self.r_edges) * 100) + "%"
            axins = inset_axes(
                fig.get_axes()[len(self.r_edges) // 2],
                width=wdth,
                height="15%",
                loc="lower center",
                bbox_to_anchor=(0.0, -0.2, 1, 1),
